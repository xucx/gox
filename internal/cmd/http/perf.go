...
var (
	PerfOptions = RunPerfOpts{}
	PerfCmd     = &cobra.Command{
		Use:   "perf",
		Short: "perf",
		Args:  cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			PerfOptions.Url = args[0]
			return RunPerf(cmd.Context(), PerfOptions)
		},
	}
)
...
func init() {
	flags := PerfCmd.Flags()
	flags.Uint32VarP(&PerfOptions.Concurrency, "concurrency", "c", 1, "concurrency")
	flags.Uint32VarP(&PerfOptions.Number, "number", "n", 1, "count")
	flags.StringVarP(&PerfOptions.Method, "method", "x", "GET", "http method")
	flags.BoolVarP(&PerfOptions.Keepalive, "keepalive", "k", false, "keep alive")
	flags.IntVarP(&PerfOptions.Timeout, "timeout", "t", 30, "time out")
	flags.StringArrayVarP(&PerfOptions.Headers, "header", "H", []string{}, "http header")
	flags.StringVarP(&PerfOptions.Body, "body", "d", "", "http body")
}

const epsilon = 1e-9

type RunPerfOpts struct {
	Concurrency uint32
	Number      uint32
	Method      string
	Url         string
...
func RunPerf(ctx context.Context, options RunPerfOpts) error {
...
func newClientCalculater(options *RunPerfOpts) *clientCalculater {
...
func newClientWorker(options *RunPerfOpts, ch chan *sendResult, index int, req *http.Request, count uint32) *clientWorker {
	return &clientWorker{
		workerIndex: index,
		options:     options,
		ch:          ch,
...
func newHttpClient(options *RunPerfOpts) *http.Client {
	return &http.Client{
